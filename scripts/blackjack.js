...
let playerLossesEl = document.getElementById("player-losses");
...
function checkForEndOfGame() {
  ...
  updateRecord(playerWon, gameOver);
}

function updateRecord(playerWon, gameOver) {
  if (playerWon === true) {
    playerWins = parseInt(localStorage.getItem("playerWins"));
  ...
  } else if (playerWon === false && gameOver === true) {
    playerLosses = parseInt(localStorage.getItem("playerLosses"));

    if (playerLosses) {
      playerLosses++;
      localStorage.setItem("playerLosses", playerLosses);
    } else {
      localStorage.setItem("playerLosses", 1);
    }
  }
}

function showStatus() {
  winRecord = localStorage.getItem("playerWins");
  loseRecord = localStorage.getItem("playerLosses");

  playerWinsEl.innerText = winRecord;
  playerLossesEl.innerText = loseRecord;
  ...
  if (gameOver) {
    ...
    stayButton.style.display = "none";
    textArea.innerText =
      'Game ended, to start a new game press "New Game" below.';
  }
}
